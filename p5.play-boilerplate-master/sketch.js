...
function draw() {
  ...
  if(gameState===PLAY){
        ...
        dash.overlap(planetsGroup,function(collector,collected){
                collected.remove();
                heartsGroup[heartsGroup.length-1].destroy();
        })
        //console.log(heartsGroup)
        // dash.overlap(planetsGroup,function(collector,collected){
        //         collected.remove();
        //         heartsGroup[heartsGroup.length-1].remove();
        // })
        // dash.overlap(planetsGroup,function(collector,collected){
        //         collected.remove();
        //         heartsGroup[heartsGroup.lenght-1].remove();
        // })
        

         if(heartsGroup.length===0)
         {
                gameState===END;
         }
...
function spawnObstacles()
{
  if(frameCount%260===0){
    ...
    planet.velocityX+=(4+frameCount/180)
